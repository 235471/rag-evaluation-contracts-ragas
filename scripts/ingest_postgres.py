#!/usr/bin/env python3
"""
CLI script to ingest documents into PostgreSQL/PGVector.
Loads PDFs, chunks, embeds, and adds to vectorstore with retry/dedup.

Usage:
    python scripts/ingest_postgres.py
    python scripts/ingest_postgres.py --documents-dir ./my_docs --table my_table
    python scripts/ingest_postgres.py --batch-size 15 --base-delay 30
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest documents into PostgreSQL/PGVector"
    )
    parser.add_argument(
        "--documents-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=15.0,
        help="Base delay for retry backoff in seconds (default: 15.0)",
    )
    parser.add_argument(
        "--max-retries",
        type=int,
    ...
    try:
        ...
        # Add content hashes
        from src.app.utils.dedup import add_content_hash_to_docs

        docs = add_content_hash_to_docs(chunks)

        # Verify hashes
        for doc in docs:
            assert doc.metadata.get("content_hash"), "Missing content_hash"
